...
//put following line in place of comment line above if wanting memcache
...
app.get("/", (req, res) => {
 ...
 Promise.all([getMissions(), getPopular(topX)]).then((values) => {
  ...
  const popularHtml = popular
   .map(
    (pop) =>
     `<li> <a href='missions/${pop.mission}'>${pop.mission}</a> (${pop.count} views) </li>`
   )
   .join("\n");
...
async function getMission(mission) {
 const query =
  "SELECT mission, heading, description FROM missions WHERE mission = ?";
...
app.get("/missions/:mission", (req, res) => {
 ...
 sendTrackingMessage({
  ...
  .then(() =>
   console.log(
    `Sent mission=${mission} to kafka topic=${options.kafkaTopicTracking}`
   )
  )
 ...
 getMission(mission)
  .then((data) => {
   sendResponse(
    res,
    `<h1>${data.mission}</h1><p>${data.heading}</p>` +
     data.description
      .split("\n")
      .map((p) => `<p>${p}</p>`)
      .join("\n"),
